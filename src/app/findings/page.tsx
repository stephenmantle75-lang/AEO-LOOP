  ...
  return <ObservatoryShell active="findings" findingCount={findingCount}>
    ...
    <section className="panel page-panel">
      ...
      {drafts.length ? <div className="evidence-stack">{drafts.map((draft) => <article className="evidence-card" key={draft.id}>
        ...
        <div className="evidence-meta"><span>Confidence {Math.round(draft.confidence * 100)}%</span><span>{draft.evidenceIds.length} evidence row{draft.evidenceIds.length === 1 ? "" : "s"}</span><Link className="detail-link" href={`/runs/${draft.runId}`}>Open source run →</Link><Link className="detail-link" href={`/review/${draft.runId}`}>Review draft →</Link></div>
    ...
    <section className="panel page-panel">
      ...
      {analysis ? <>
        ...
        <div className="notice">This analysis is review-only. Its findings are linked to stored observation IDs. A deterministic snapshot can be persisted behind the disabled-by-default analysis flag; human approval, model-backed analysis, and external delivery remain deferred.</div>
    ...
    <section className="panel architecture"><div className="panel-title">What happens next</div><div className="flow"><div className="flow-step">Stored evidence<br /><small>Supabase rows</small></div><div className="arrow">→</div><div className="flow-step">Draft analysis<br /><small>deterministic rules</small></div><div className="arrow">→</div><div className="flow-step">Optional snapshot<br /><small>flagged, local gate</small></div><div className="arrow">→</div><div className="flow-step">Human review<br /><small>approve / reject</small></div><div className="arrow">→</div><div className="flow-step">Linear + PR<br /><small>later phase</small></div></div><div className="notice">The analysis snapshot is additive and disabled by default. It still cannot create Linear issues, Slack messages, portfolio changes, or deployments. Human approval remains the boundary for action.</div></section>
  </ObservatoryShell>;
}
