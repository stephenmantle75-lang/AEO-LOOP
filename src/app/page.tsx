...
import { citationRate, displayUrl, formatCost, formatDate, formatDuration, getOverviewData, observationLabel, providerLabel, runLabel } from "@/lib/observatory";
...
export default async function ObservatoryPage() {
  ...
  const { runs, findings, observationCount, latestObservations } = result.data;
  const latest = runs[0];
  ...
  return <ObservatoryShell active="overview" findingCount={openFindings}>
    ...
    <section className="cards" aria-label="Key metrics">
      ...
      <div className="card"><div className="card-label">Evidence captured</div><div className="metric">{observationCount}</div><div className="metric-note">observations stored in Supabase</div></div>
