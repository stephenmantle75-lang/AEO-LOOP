...
import { buildTopicRunSnapshots } from "@/lib/reporting";
...
export default async function TopicDetailPage({ params }: Props) {
  ...
  const runSnapshots = buildTopicRunSnapshots(observations);
  return <ObservatoryShell active="topics" findingCount={findingCount}>
    ...
    <section className="panel page-panel"><div className="panel-head"><span className="panel-title">Evidence history</span><span className="panel-meta">{observations.length} records from Supabase</span></div>{observations.length ? <div className="table-wrap"><table><thead><tr><th>Observed</th><th>Provider</th><th>Check</th><th>Status</th><th>Result</th></tr></thead><tbody>{observations.map((observation) => <tr key={observation.id}><td>{formatDate(observation.observed_at)}</td><td><strong>{providerLabel(observation.provider)}</strong></td><td>{observationLabel(observation.observation_type)}<small>{observation.question}</small></td><td><span className="run-status"><span className={`dot ${observation.status}`} />{statusLabel(observation.status)}</span></td><td>{observation.status === "failed" ? <span className="result-error">{publicErrorMessage(observation.error_message, "Provider failed; inspect server logs")}</span> : observation.provider === "exa" ? <span className={observation.citation_found ? "result-positive" : "result-muted"}>{observation.citation_found ? "Citation found" : "No citation found"}</span> : <span className="result-positive">Document inspected</span>}</td></tr>)}</tbody></table></div> : <div className="empty"><strong>No evidence has been recorded yet</strong>The topic is configured, but no provider result is available.</div>}</section>
    <section className="panel page-panel"><div className="panel-head"><span className="panel-title">Run history</span><span className="panel-meta">same topic · denominator preserved</span></div>{runSnapshots.length ? <div className="table-wrap"><table><thead><tr><th>Observed</th><th>Run</th><th>Observed checks</th><th>Cited</th><th>Rate</th><th>Health</th></tr></thead><tbody>{runSnapshots.map((snapshot) => <tr key={snapshot.runId}><td>{formatDate(snapshot.observedAt)}</td><td><Link className="detail-link" href={`/runs/${snapshot.runId}`}>{snapshot.runId.slice(0, 8)}…</Link></td><td>{snapshot.observedChecks || "—"}</td><td>{snapshot.observedChecks ? `${snapshot.citedChecks}/${snapshot.observedChecks}` : "—"}</td><td>{snapshot.citationRate === null ? "—" : `${Math.round(snapshot.citationRate * 100)}%`}</td><td>{snapshot.failedObservations ? <span className="result-error">{snapshot.failedObservations} failed</span> : <span className="result-positive">No failures</span>}</td></tr>)}</tbody></table></div> : <div className="empty"><strong>No run history yet</strong>Repeated real collections will appear here so control and variant results can be compared with their denominators intact.</div>}<div className="notice">This is a measurement history, not a causal claim. A page improvement is only evaluated after the same prompt set is collected again against the approved variant.</div></section>
  </ObservatoryShell>;
}
